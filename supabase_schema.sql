...
-- ==========================================
-- SUPABASE SQL SCHEMA FOR INVENTORY & HISTORY
-- ==========================================

-- 9. Table: inventory (Stok Barang)
CREATE TABLE IF NOT EXISTS public.inventory (
    id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    nama_barang text NOT NULL,
    kategori text NOT NULL, -- 'Tinta', 'Sparepart', dll
    stok integer NOT NULL DEFAULT 0,
    harga_satuan text DEFAULT '0',
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
-- Create Select Policy (Everyone can read if needed for form dropdowns, or just admin)
...
CREATE POLICY "Admin can insert inventory" ON public.inventory FOR INSERT WITH CHECK (auth.role() = 'authenticated');
DROP POLICY IF EXISTS "Admin can update inventory" ON public.inventory;
CREATE POLICY "Admin can update inventory" ON public.inventory FOR UPDATE USING (auth.role() = 'authenticated');
DROP POLICY IF EXISTS "Admin can delete inventory" ON public.inventory;
CREATE POLICY "Admin can delete inventory" ON public.inventory FOR DELETE USING (auth.role() = 'authenticated');
...
-- Update service_tracking for JSON spareparts
ALTER TABLE public.service_tracking ADD COLUMN IF NOT EXISTS spareparts_terpakai JSONB DEFAULT '[]'::jsonb;
ALTER TABLE public.service_tracking ADD COLUMN IF NOT EXISTS waktu_selesai timestamp with time zone;

-- PostgreSQL Function to reduce stock automatically when status is 'Diambil'
CREATE OR REPLACE FUNCTION reduce_stock_on_complete()
...
DECLARE
    ...
    item_qty int;
BEGIN
    -- Only trigger if status just changed to 'Diambil'
    IF NEW.status = 'Diambil' AND OLD.status != 'Diambil' THEN
        -- Set waktu_selesai
        NEW.waktu_selesai = now();
        
        -- Loop through the JSONB array (e.g., [{"id": 1, "qty": 2}])
        IF NEW.spareparts_terpakai IS NOT NULL AND jsonb_typeof(NEW.spareparts_terpakai) = 'array' THEN
            FOR item IN SELECT * FROM jsonb_array_elements(NEW.spareparts_terpakai)
            LOOP
...
DROP TRIGGER IF EXISTS trigger_reduce_stock ON public.service_tracking;
CREATE TRIGGER trigger_reduce_stock
BEFORE UPDATE ON public.service_tracking
FOR EACH ROW
EXECUTE PROCEDURE reduce_stock_on_complete();
